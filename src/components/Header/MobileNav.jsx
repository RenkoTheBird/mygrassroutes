...
export default function MobileNav({ navLinks, actions, activeDropdown, toggleDropdown, closeMenu }) {
  return (
    <div className="md:hidden bg-emerald-700 text-white px-4 py-4 space-y-2">
      {navLinks.map((link, index) => {
        if (link.children && link.children.length > 0) {
          return (
            <div key={index}>
              <button
                className="flex items-center justify-between w-full px-3 py-2 hover:bg-emerald-600 rounded"
